...
*   -  "동적" 이라는 단어는 프로그램 실행/문제풀이 중 데이터를 저장한다는 의미
*  */
public class A_Fibonacci {
    ...
    * 필요한 부분만 계산
    * 재쉬 함수의 직관성 + DP의 효율 확인
    * Memoization(메모이제이션)
    *   - 작은 문제 결과를 메모리에 저장
    *   - 이전에 계산한 결과를 재활용하는 기법
    * */
    static int[] DP = new int[50]; // Memo를 하기 위한 공간
    ...
    *   - 모든 부분을 순차적으로 계산
    *   - 재귀 호출X -> Stack Overflow 방지
    *   - 일반적으로 top-down 방식보다 약간 더 빠름
    *
    * tabulation
    *   - 작은 문제로 나눈 후 연산 결과를 테이블(배열)에 저장하는 방식
    * */

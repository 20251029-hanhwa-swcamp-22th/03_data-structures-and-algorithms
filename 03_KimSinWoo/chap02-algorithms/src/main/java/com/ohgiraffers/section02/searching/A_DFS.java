...
*   - 재귀 호출 또는 Stack(후입 선출)를 이용하여 구현 가능
*   - 시작 노드에서 출발하여 한 방향으로만 갈 수 있는 만큼 깊이 탐색한 후 더 이상 진행할 수 없을 때,
*     이전 분기점 노드로 돌아가 다른 경로를 탐색하는 알고리즘.
* */
public class A_DFS {

    /* 웜 바이러스 걸리게되는 컴퓨터 수 구하기 */
    static int node; // 컴퓨터(node)의 총 개수
    static int edge; // 네트워크(간선)의 총 개수
    ...
    public static Integer solution(String input) throws IOException {
        ...
        // 노드 방문 여부를 저장할 배열 생성
        // - 방문하면 true로 변환 (visit[i] = true --> i 노드 방문한적  있음)
        ...
        // 1. 재귀 호출 방법
//        dfsRecursive(1); // 1번 부터 탐색을 시작

        // 2. stack 활용 방법
        dfsStack(1);
        return count;  // 감염된 컴퓨터(node)의 개수 반환
    }
    ...
    private static void dfsRecursive(int start) {
        /* 1단계: 해당 노드를 방문 했으므로 방문 배열에 표기 */
        visit[start] = true;

        /* 2단계: start 노드의 모든 이웃을 탐색하는 과정 */
        for (int i = 1; i < node; i++) {
            /* start 노드와 연결되어 있고 아직 방문한적이 없는 이웃노드 찾기 */
            if (map[start][i] == 1 && !visit[i]) {
                count++;

                /* 3단계: 찾은 이웃 노드를 방문해서 그 노드와 연결된 이웃을 또 찾는 과정 반복(재귀 호출)

...
public class E_MergeSort {
    ...
    private static void mergeSort(int[] arr, int[] temp, int left, int right) {
        // 분할된 배열의 크기가 2이상인 경우에만 동작
        // 분할된 배열의 크기가 1이하인 경우 수행 X
        // if문 종료 == 분할된 배열 정렬 완료
        if (left < right) {
            // 1. 분할 - 중간 지점 계산
            int mid = (left + right) / 2;

            // 2. 왼쪽 정복 - 왼쪽 부분 재귀 호출(left ~ mid)
            mergeSort(arr, temp, left, mid);

            // 3. 오른쪽 정복 - 오른쪽 부분 재귀 호출(mid+1 ~ right)
     ...
     * @param mid 왼쪽 부분 배열의 끝 인덱스, 오른쪽 부분 배열의 시작 인덱스-1
    ...
    private static void merge(int[] arr, int[] temp, int left, int mid, int right) {
        ...
        // 1 단계: 병합할 구간의 배열 요소를 임시 배열(temp)에 복사
        // -> 원본 수정 전 값을 보존하기 위해서
        ...
        // 3 단계: 두 부분 배열을 비교하면서 작은 값부터 원본 배열에 저장하겠다
        // -> 이 과정에서 정렬이 됨
        // 두 부분 배열에 모두 남은 요소가 있을 때만 반복
        while(leftIndex <= mid && rightIndex <= right) {

            // 임시 배열의 leftIndex가 가리키는 값이 rightIndex가 가리키는 값 보다 같거나 작은 경우
            if (temp[leftIndex] <= temp[rightIndex]) {
                arr[current++] = temp[leftIndex++];
            }
            // 임시 배열의 rightIndex가 가리키는 값이 leftIndex가 가리키는 값 보다 같거나 작은 경우
            else {
